...
def build_calibration_artifacts(root: Path | None = None, destination: Path | None = None) -> dict[str, Any]:
    ...
    calibration = [{"archetype_id": item["archetype_id"], "archetype_source_hash": _hash(item), "descriptor_refs": [profile["style_profile_id"]], "measured_metrics": [{"metric_name": "text_budget", "observed_value": item["text_budget"], "target_range": [item["text_budget"], item["text_budget"]], "evidence_tier": "structurally_supported", "status": "provisional"}, {"metric_name": "asset_capacity", "observed_value": item["asset_limits"]["max"], "target_range": [item["asset_limits"]["min"], item["asset_limits"]["max"]], "evidence_tier": "structurally_supported", "status": "provisional"}], "structural_geometry": "provisional", "style_token_state": "provisional", "render_state": "blocked_environment", "qualitative_state": "blocked_visual_review", "reason": "measured canonical archetype constraints; no qualitative professor acceptance"} for item in items]
    names = ("fishbone","mechanism","experiment","fabrication","comparison","scientific_plot","image_matrix","concept_illustration")
    family = {"schema_version":"1.0.0","families":[{"family":name,"evidence_state":"provisional","representative_fixture":{"fixture_id":f"G1-{name}-REP","input_kind":"synthetic_structural_fixture","expected_invariants":["safe_bounds","stable_ids"]},"stress_fixture":{"fixture_id":f"G1-{name}-STRESS","input_kind":"synthetic_structural_fixture","expected_invariants":["safe_bounds","density_or_cardinality"]},"evaluation_result":{"status":"provisional","safe_bounds_result":"pass","render_result":"not_measured"},"representative_benchmark_count":1,"stress_benchmark_count":1} for name in names]}
    fishbone = {"schema_version":"1.0.0","profile_id":"CP5G-FISHBONE-001","fields":{"spine_placement":{"status":"provisional","value":"centerline","evidence_ref":"D1-fishbone-layout"},"branch_spacing":{"status":"provisional","value":"depth_ordered","evidence_ref":"D1-fishbone-layout"},"label_placement":{"status":"provisional","value":"adjacent_node","evidence_ref":"D1-fishbone-layout"},"focus_emphasis":{"status":"provisional","value":"generic_emphasis_role","evidence_ref":"VSP003"},"completed_status":{"status":"insufficient_evidence"},"partial_status":{"status":"insufficient_evidence"},"failed_status":{"status":"insufficient_evidence"},"future_status":{"status":"insufficient_evidence"},"safe_bounds":{"status":"provisional","value":"0.05_to_0.95","evidence_ref":"D1-fishbone-layout"}},"material_semantic_colors":"unresolved"}
    qa = {"schema_version":"1.0.0","qa_id":"CP5G-QA-001","structural_geometry_calibration":"provisional","style_token_calibration":"provisional","render_calibration":"blocked_environment","image_capable_qualitative_calibration":"blocked_visual_review","professor_visual_acceptance":"blocked","private_alias_resolution_attempts":0,"private_source_open_attempts":0,"private_render_attempts":0,"archetype_count":len(calibration),"measured_metric_count":sum(len(item["measured_metrics"]) for item in calibration),"style_profile_ref":profile["style_profile_id"]}
    if destination:
        destination.mkdir(parents=True,exist_ok=True); folder=destination/"cp5g";folder.mkdir(exist_ok=True)
        artifacts={"archetype-calibration.json":{"schema_version":"1.0.0","archetypes":calibration},"figure-family-calibration.json":family,"fishbone-style-profile.json":fishbone,"reconstruction-benchmarks.json":{"schema_version":"1.0.0","benchmarks":[{"family":x["family"],"representative_fixture":x["representative_fixture"],"stress_fixture":x["stress_fixture"],"evaluation_result":x["evaluation_result"]} for x in family["families"]]},"checkpoint-5g-execution-evidence.json":{"schema_version":"1.0.0","execution_id":"CP5G-EXEC-001","private_alias_resolution_attempts":0,"private_source_open_attempts":0,"private_render_attempts":0},"checkpoint-5g-qa.json":qa}
        for name,value in artifacts.items():(destination/name).write_text(json.dumps(value,ensure_ascii=False,indent=2)+"\n",encoding="utf-8")
        archetype_panels = ''.join(f'<g id="obj-a{index}" data-semantic-role="panel" transform="translate({60+(index%6)*250} {140+(index//6)*220})"><rect id="obj-a{index}-box" data-semantic-role="container" x="0" y="0" width="210" height="160" fill="#f4f4f4" stroke="#333333"/><text id="obj-a{index}-label" data-semantic-role="label" x="18" y="38" font-family="Arial" font-size="22">{item["archetype_id"]}</text><line id="obj-a{index}-split" data-semantic-role="connector" x1="18" y1="65" x2="192" y2="65" stroke="#333333"/><rect id="obj-a{index}-figure" data-semantic-role="panel" x="18" y="82" width="110" height="56" fill="#d1d1d1"/></g>' for index,item in enumerate(calibration))
        archetype_board=f'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1600 900" data-thesis-svg-version="1.0.0" data-thesis-figure-id="FIG010" data-visual-class="conceptual_explanation"><text id="obj-archetype-title" data-semantic-role="title" x="60" y="90" font-family="Arial" font-size="36">A01–A18 structural calibration geometry</text>{archetype_panels}</svg>'
        family_panels = ''.join(f'<g id="obj-family-{index}" data-semantic-role="panel" transform="translate({80+(index%4)*380} {160+(index//4)*300})"><rect id="obj-family-{index}-box" data-semantic-role="container" x="0" y="0" width="320" height="220" fill="#ffffff" stroke="#333333"/><polyline id="obj-family-{index}-shape" data-semantic-role="flow" points="30,170 130,80 260,170" fill="none" stroke="#FF0000" stroke-width="4"/><text id="obj-family-{index}-label" data-semantic-role="label" x="25" y="35" font-family="Arial" font-size="22">{item["family"]} rep/stress</text></g>' for index,item in enumerate(family["families"]))
        family_board=f'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1600 900" data-thesis-svg-version="1.0.0" data-thesis-figure-id="FIG010" data-visual-class="conceptual_explanation"><text id="obj-family-title" data-semantic-role="title" x="80" y="90" font-family="Arial" font-size="36">Figure-family representative and stress calibration</text>{family_panels}</svg>'
        (folder/"archetype-calibration-montage.svg").write_text(archetype_board,encoding="utf-8");(folder/"figure-family-calibration-montage.svg").write_text(family_board,encoding="utf-8")
