...
def build():
    ...
    first_cursor=len(ledger.replay()); ledger.serialize(ARTIFACTS/"ledger-events.json"); persisted=Ledger.load(ARTIFACTS/"ledger-events.json"); first_state=persisted.materialize(); write(ARTIFACTS/"materialized-first.json",first_state)
    obs=state_content(first_state,"observation",rel(obs_svg))
    res=state_content(first_state,"discussion",rel(plot["svg"]))
    ...
    revised_state=persisted.materialize(); revised_obs=state_content(revised_state,"observation",rel(obs_svg)); revised_result=state_content(revised_state,"discussion",rel(plot["svg"]))
    specs2=[compile_slide("B001","observation","photo_observation",revised_cursor,revision=2,content=revised_obs,asset_path=rel(obs_svg),asset_id="A002"),compile_slide("B001","result","hero_plot_discussion",revised_cursor,revision=2,content=revised_result,asset_path=rel(plot["svg"]),asset_id="A001")]; write(ARTIFACTS/"slide-specs-revised.json",specs2); revised=ARTIFACTS/"master_revised_build.pptx"; asm.assemble(template,specs2,revised); asm.assemble(template,specs1,ARTIFACTS/"master_first_render_compat.pptx",attach_svg=False); asm.assemble(template,specs2,ARTIFACTS/"master_revised_render_compat.pptx",attach_svg=False)
    ...
    bundle["slide_specs"]=specs1+specs2; bundle["deck_manifests"]=[json.loads((ARTIFACTS/"MASTER-PHASE1-FIRST.manifest.json").read_text(encoding="utf-8")),json.loads((ARTIFACTS/"MASTER-PHASE1-REVISED.manifest.json").read_text(encoding="utf-8"))]; bundle["template_profiles"]=[tp]; bundle["meeting_projection"]=md; bundle["_materialized_paths"]={"first":str(ARTIFACTS/"materialized-first.json"),"revised":str(ARTIFACTS/"materialized-revised.json"),"first_cursor":first_cursor}
    audit=audit_pptx(revised,template,tp,specs2); qa=run_pipeline(bundle=bundle,ledger=persisted,specs=specs2,structural_audit=audit,native_available=False,professor_profile=bundle["professor_profiles"][0],render_evidence={}); qa.update({"artifacts":{"pptx":rel(revised),"template_profile":rel(ARTIFACTS/"template-profile.json"),"plot_svg":rel(plot["svg"]),"plot_png":rel(plot["png"]),"meeting_delta":rel(ARTIFACTS/"meeting-delta.json")},"structural_audit":audit,"native_status":"blocked_environment"}); write(ARTIFACTS/"qa-report.json",qa)
    return {"first":first,"revised":revised,"qa":qa,"ledger":persisted}

def finalize_visual_qa():
    bundle=load_fixture(PROJECT); bundle["assets"]=[json.loads((ARTIFACTS/"plots/A001.asset.json").read_text(encoding="utf-8")),json.loads((ARTIFACTS/"plots/A002.asset.json").read_text(encoding="utf-8"))]; bundle["slide_specs"]=[*json.loads((ARTIFACTS/"slide-specs-first.json").read_text(encoding="utf-8")),*json.loads((ARTIFACTS/"slide-specs-revised.json").read_text(encoding="utf-8"))]; bundle["deck_manifests"]=[json.loads((ARTIFACTS/"MASTER-PHASE1-FIRST.manifest.json").read_text(encoding="utf-8")),json.loads((ARTIFACTS/"MASTER-PHASE1-REVISED.manifest.json").read_text(encoding="utf-8"))]; bundle["template_profiles"]=[json.loads((ARTIFACTS/"template-profile.json").read_text(encoding="utf-8"))]; bundle["_repo_root"]=str(ROOT); bundle["_schema_dir"]=str(ROOT/"thesis-deck-system/schemas")
    specs=json.loads((ARTIFACTS/"slide-specs-revised.json").read_text(encoding="utf-8")); ledger=Ledger.load(ARTIFACTS/"ledger-events.json"); tp=json.loads((ARTIFACTS/"template-profile.json").read_text(encoding="utf-8")); audit=audit_pptx(ARTIFACTS/"master_revised_build.pptx",ARTIFACTS/"synthetic_native_template.pptx",tp,specs); bundle["meeting_projection"]=json.loads((ARTIFACTS/"meeting-delta.json").read_text(encoding="utf-8")); bundle["_materialized_paths"]={"first":str(ARTIFACTS/"materialized-first.json"),"revised":str(ARTIFACTS/"materialized-revised.json"),"first_cursor":json.loads((ARTIFACTS/"meeting-delta.json").read_text(encoding="utf-8"))["since_cursor"]}
    inspection={"checked_by":"Codex visual inspection","slides":[{"slide_id":"TEMPLATE-SLIDE-01","render_path":rel(ARTIFACTS/"render_revised/slide-1.png"),"checks":["nonblank","legible","no clipping"],"observations":"Synthetic template title and subtitle are centered, readable, and unclipped.","status":"pass"},{"slide_id":"TEMPLATE-SLIDE-02","render_path":rel(ARTIFACTS/"render_revised/slide-2.png"),"checks":["nonblank","legible","native placeholder visible"],"observations":"Representative native title/content placeholders render clearly without overlap.","status":"pass"},{"slide_id":"S-B001-OBSERVATION-01","render_path":rel(ARTIFACTS/"render_revised/slide-3.png"),"checks":["nonblank","legible","visual present","no overlap"],"observations":"Synthetic observation visual, observation statement, and problem statement are visible and separated.","status":"pass"},{"slide_id":"S-B001-RESULT-01","render_path":rel(ARTIFACTS/"render_revised/slide-4.png"),"checks":["nonblank","legible","plot and next step present","no overlap"],"observations":"Plot, D002 rationale, revised discussion, NS001 status, and due time are visible without collision.","status":"pass"}]}; write(ARTIFACTS/"visual-inspection.json",inspection)
    renders=sorted((ARTIFACTS/"render_revised").glob("slide-*.png")); visual={"inspection_record":rel(ARTIFACTS/"visual-inspection.json"),"render_paths":[rel(p) for p in renders],"montage_paths":[rel(ARTIFACTS/"render_revised/full-deck-montage.png"),rel(ARTIFACTS/"render_revised/changed-slide-montage.png")]}
